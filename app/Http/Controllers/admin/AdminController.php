...
class AdminController extends Controller
{
    ...
    */
    public function index() {
        $data['superadmin_ct'] = User::where('role', 'superadmin')->count();
        $data['admin_ct'] = User::where('role', 'admin')->count();
        $data['user_ct'] = User::where('role', 'user')->count();
    ...
    */
    public function addUser(Request $request) {
    	...
    	if ($validator->fails()) { 
			...
			if($user->save()) {
                $adm = User::where('role', 'admin')->first(); $email = $adm->email;
                $data = array('name' => $request['name'], 'email' => $request['email'], 'username' => $request['username'], 'password' => $request['password'], 'role' => $request['role']);
                // send mail to user.
    ...
    */
    public function updateUser(Request $request) {
    	...
    	if(User::where('email', $request['email'])->where('id', '!=', $request['id'])->count() > 0) {
	    	...
	    	User::where('id', $request['id'])->update([ 'name' => $request['name'], 'email' => $request['email'], 'username' => $request['username'], 'role' => $request['role'] ]);
